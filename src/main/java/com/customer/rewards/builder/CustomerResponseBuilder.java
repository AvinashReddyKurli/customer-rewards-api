	...
	private void calculateRewards(TransactionPeriod transacationPeriod) {
		transacationPeriod.getCustomers().forEach(customer -> {
			customer.getTransactions().forEach(transaction -> {
				int rewards = 0;
				if(transaction.getDollersSpent() > HUNDRED ) {
					rewards += (transaction.getDollersSpent() - HUNDRED) * 2;
				} else if(transaction.getDollersSpent() > FIFTY && transaction.getDollersSpent() <= HUNDRED) {
					rewards += (transaction.getDollersSpent() - FIFTY) * 1;
				}
